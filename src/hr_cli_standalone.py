...
# 相対インポートを避けて直接インポート
import hr_recruitment_system as hrs
import interview_system as ivs
...
class HRCLISystem:
    ...
    def __init__(self):
        ...
        self.analyzer = hrs.ResumeAnalyzer()
        self.question_generator = ivs.InterviewQuestionGenerator()
    ...
    def setup_company_profile(self):
        ...
        company_profile = hrs.CompanyProfile(
    ...
    def load_company_profile(self) -> Optional[hrs.CompanyProfile]:
        """企業プロファイルを読み込み"""
        ...
        try:
            with open(self.company_config_path, 'r', encoding='utf-8') as f:
                data = json.load(f)
            return hrs.CompanyProfile(**data)
    ...
    def analyze_resume(self, resume_file_path: str) -> hrs.CandidateProfile:
        """履歴書を分析"""
    ...
    def evaluate_candidate(self, candidate: hrs.CandidateProfile, job_position: str) -> hrs.MatchingResult:
        """候補者を評価"""
        ...
        job_req = hrs.JobRequirement(**jobs_data[job_position])
        
        matcher = hrs.CandidateMatcher(company)
        matching_result = matcher.calculate_match_score(candidate, job_req)
        
        return matching_result
    
    def generate_interview_plan(self, candidate: hrs.CandidateProfile, job_position: str, stage: str) -> ivs.InterviewPlan:
        """面接計画を生成"""
        stage_map = {
            "1st": ivs.InterviewStage.FIRST,
            "2nd": ivs.InterviewStage.SECOND,
            "final": ivs.InterviewStage.FINAL
        }
    ...
    def print_evaluation_result(self, matching_result: hrs.MatchingResult):
        """評価結果を出力"""
        print("\n" + "="*60)
    ...
    def print_interview_plan(self, interview_plan: ivs.InterviewPlan):
        """面接計画を出力"""
        print("\n" + "="*60)
